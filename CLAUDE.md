...
- `src/renderer/stores/` — 投影层（反映世界，不进流水线）。editor-store 是零 diff 纪律的归宿：frontmatter 只做行级手术（shared/markdown-utils）、body 只在真改过时才走 Crepe 序列化、没编辑就不写盘；回声去重 = 与 lastWritten 精确比对。依赖方向：editor-store 不许 import project-store（反向可以）
