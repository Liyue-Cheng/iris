...
- [ ] `npm run build` 后按 electron-builder 出 portable 包，双击启动 → 不再出现 GPU cache 报错。
- [ ] 若仍报错：下一步换思路——用 `app.setPath('sessionData', <可写目录>)` 或 `--disk-cache-dir` 把缓存显式指到可写位置（如 `%LOCALAPPDATA%/Iris` 或 portable 包同级的可写子目录）。
- [ ] 确认禁用 shader 缓存后无可感知的渲染性能退化。
