...
### 哑壳 + 智能外包
应用本身近乎零智能。agent 能力来自用户本机已安装登录的 CLI(claude / codex / gemini / opencode / aider……)。App 默认不内嵌 agent、SDK 或 API key——智能外包是基本姿态,不是禁令。坚持的是**不做 agent、不做订阅**;不拒绝的是**用户 BYOK 驱动的辅助功能**(如自动填标签):壳不自带智能,不等于拒绝用户自带的智能。
...
- 新开一个终端会话,工作目录为项目根,环境变量 `FOCUS_DOC=<被选中文档的相对路径>`,**裸启动** agent(不传任务 prompt;可经用户自己的 agent 配置注入上下文)。
- agent 按注入链读宪法 → 按协议读 `$FOCUS_DOC` → 上下文到手 → **停住等用户指令**。打开 ≠ 开跑:没有 user message 就没有任务。
- **上下文注入是渲染层适配,不是协议**:壳依然只设 `FOCUS_DOC`。有 SessionStart hook 的 agent 用 hook 把指针变成内容(零轮次,内容进上下文但模型不被调用),没 hook 的用启动 flag;hook 配置住在用户自己的 agent 配置文件里,Iris 只检测、建议、经确认代写。两者皆无的 agent 降级回"按 AGENTS.md 引导自己读"——协议本来就允许。
...
- 不内嵌 agent、SDK、API key——用户自带 CLI、自带计费,壳保持哑。**这不排斥用户 BYOK 驱动的辅助功能**(用户自带 key、自担计费的小自动化,如自动填标签)——禁的是壳自带智能,不是用户自带智能。
...
- ~~v1 不做多项目管理——一次打开一个项目。~~ **v1.1 起支持 VS Code 式多窗口/多项目**（产品负责人 2026-06-23 拍板）：一个主进程 + N 个窗口，每窗口绑一个项目、各自拥有 ProjectManager/SessionManager/文件监听；机器级设置仍全局共享。详见 issue「多窗口/多项目（v1.1）」。
