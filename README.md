...
<p align="center">
  <strong>鸢尾工坊</strong><br>
  让项目记忆留在项目里。
</p>

鸢尾工坊（Iris）是一个以持久项目记忆为核心的本地优先人机协同开发工作空间。它将项目文档、Agent、Git 状态和人的注意力整合进同一个桌面工作空间，不提供模型服务，也不取代你已有的 Agent CLI；它为这些工具提供持久的项目记忆，以及一个让人与 Agent 共同工作的可视化空间。
...
![鸢尾工坊工作区](docs/images/readme-overview.png)
...
鸢尾工坊完全通过自身工作流进行开发。本仓库的 [`.iris/`](./.iris/) 目录是真实的项目记录，而不是示例：issue 保存意图和调查过程，status 映射当前实现，report 保留有时间属性的结论，任务复选框则让尚待人处理的决策保持可见。
